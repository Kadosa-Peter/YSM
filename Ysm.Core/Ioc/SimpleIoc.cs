    ...
    public class SimpleIoc : ISimpleIoc
    {
            ...
            where TClass : class
        {
            lock (_syncLock)
            {
                ...
                if (_instancesRegistry.ContainsKey(classType))
                {
                    Dictionary<string, object> list = _instancesRegistry[classType];
        ...
        private TClass MakeInstance<TClass>()
        {
            ...
            ConstructorInfo constructor = _constructorInfos.ContainsKey(serviceType)
        ...
        public IEnumerable<object> GetAllInstances(Type serviceType)
        {
            lock (_factories)
            {
                if (_factories.ContainsKey(serviceType))
                {
                    foreach (KeyValuePair<string, Delegate> factory in _factories[serviceType])
                    {
